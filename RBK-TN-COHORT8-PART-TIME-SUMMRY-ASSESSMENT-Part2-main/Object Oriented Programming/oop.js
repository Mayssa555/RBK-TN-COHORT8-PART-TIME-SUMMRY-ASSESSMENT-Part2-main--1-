...
function BookLists(read,unread,toRead,readBooks){
  this.read=read
  this.unread=unread 
  this.toRead=toRead
...
let Booklist=new BookLists(read,unread,toRead,readBooks)
...
let book= new Books(Title,Genre,Author)

Booklists.nextBook=book
Booklists.currentBook=book
Booklists.lastBook=book 

Booklist.prototype.add=function(book){
  let toRead=[]
  let read= 0
  if (this.read ==true){
      (this.toRead.push(book)) && (this.read++)
  }
}
